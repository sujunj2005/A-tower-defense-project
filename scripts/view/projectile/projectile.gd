...
## 从配置初始化弹道
func setup_from_bean():
	if not bean:
		return
	damage = bean.damage
	damage_type = bean.damage_type
	speed = bean.speed
	projectile_type = bean.movement_type
	pierce_enabled = bean.pierce_enabled
	pierce_count = bean.pierce_count
	pierce_damage_decay = bean.pierce_damage_decay
	effect_type = bean.effect_type
	effect_radius = bean.effect_radius
	effect_damage_ratio = bean.effect_damage_ratio
	effect_max_targets = bean.effect_max_targets
	var collision_shape = get_node_or_null("CollisionShape2D")
	if collision_shape and collision_shape is CollisionShape2D:
		var shape = CircleShape2D.new()
		shape.radius = bean.collision_radius
		collision_shape.shape = shape
	var sprite_node = get_node_or_null("Sprite2D")
	if sprite_node and sprite_node is Sprite2D:
		sprite_node.modulate = bean.sprite_modulate

...
func _apply_trait_effect(trait_id: String) -> void:
	...
	if config.has("effect"):
		_apply_single_effect(config.effect)
	if config.has("side_effects"):
		for side_effect: Dictionary in config.side_effects:
			_apply_single_effect(side_effect)
...
func _get_all_effects_for_trait(trait_id: String) -> Array[Dictionary]:
	var config: Dictionary = trait_configs.get(trait_id, {})
	var effects: Array[Dictionary] = []
	if config.has("effect"):
		effects.append(config.effect)
	if config.has("side_effects"):
		for se: Dictionary in config.side_effects:
			effects.append(se)
	return effects
...
func get_trait_effects_for_tower(tower_type: String) -> float:
	...
	for trait_id: String in session.traits:
		for effect: Dictionary in _get_all_effects_for_trait(trait_id):

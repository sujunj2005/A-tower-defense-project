...
> **踩坑点**：非 void 函数的所有代码路径都必须有 return 语句，否则编辑器会报错。
...
> **踩坑点**：lambda 函数必须显式使用 `return` 返回值。
...
> **踩坑点**：局部变量在 lambda 创建时按值捕获一次，之后外部修改不影响 lambda 内的值。
...
> **踩坑点**：类型数组（如 `Array[int]`）目前不支持作为 rest 参数类型。
...
> **踩坑点**：Callable 必须使用 `call()` 方法调用，不能直接用 `()`。
